...
const indikator1_cb_filter = document.getElementById("indikator1_cb_filter");
...
const handle_aspek1 = () => {
    fetch(indikator_api + "/aspek/" + aspek1_cb_filter.value)
        .then((response) => {
            if (!response.ok) {
                throw new Error("Network response was not ok");
            }
        ...
        .then((data) => {
            indikator1_cb_filter.innerHTML = "";
            data.forEach((element, i) => {
                let option = document.createElement("option");
                option.value = element.id;
                option.textContent = element.nama;
...
const create_link = (grup, indikator1, indikator2) => {
    return `${isi_api}/kmeans/${indikator1}/${indikator2}/grup/${grup}`;
};
